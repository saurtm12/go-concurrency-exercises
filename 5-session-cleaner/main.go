...
func (sm *SessionManager) cleanSessionPeriodically() {
	sm.ticker = time.Tick(time.Second)
	for {
		<-sm.ticker
		...
		for sessionID, session := range sm.sessions {
			if now.Sub(session.LastUpdated) > EXPIRATION_TIME {
				delete(sm.sessions, sessionID)
				log.Println("Deleted expired session:", sessionID)
			}
		}
		sm.mu.Unlock()
	}
